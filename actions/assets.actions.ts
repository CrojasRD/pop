...
export async function deleteAsset(id: string): Promise<ActionResult> {
  ...
  // haber borrado nada.
  const { error, data } = await supabase.from('assets').delete().eq('id', id).select('id');
  if (error) return { error: error.message };
  if (!data || data.length === 0) {
    return { error: 'No se pudo eliminar el activo. Verifica que tengas permisos de administrador o que todavía exista.' };
  }
